...
- The shared simulation shell is applied by `PhysicsLab.ui.layout()` and `PhysicsLab.draw.bg()`, so all 156 experiments receive a consistent visual stage, parameter deck, real-time readout deck, instrument grid, and labelled chart frame.
...
- Curriculum registry: 12 modules, 156 curriculum IDs, and 156 registered simulations; duplicate, missing, and extra-ID checks are all empty. The app now repeats this audit at startup and exposes the result as `window.PhysicsLabAudit` for browser QA.
...
- Added the common guided-workbench command layer and per-page access gate across the complete 156-experiment curriculum.
