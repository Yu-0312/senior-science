...
function loadCurriculum() {
  ...
  return {
    ...
    qbank: sandbox.window.PhysicsLabQuestionBank || {}
  };
}
...
/* ---------------------------------------------------------------------------
   ...
   --------------------------------------------------------------------------- */
function worksheetPage(exp, mod, site, qbank) {
  const formula = plainFormula(exp.formula);
  const rows = Array.from({ length: 6 }, (_, i) =>
    `        <tr><td>${i + 1}</td><td></td><td></td><td></td></tr>`).join("\n");
  const points = (exp.points || []).slice(0, 3).map(p =>
    `        <li><span class="ws-box"></span>${escapeHtml(p)}</li>`).join("\n");
  // 題庫第一題當「動手前先預測」：誘答全是真實迷思，圈選完進模擬驗證
  const qs = (qbank && qbank[exp.id]) || [];
  const q1 = qs.length ? qs[0] : null;
  const q1Opts = q1 ? q1.options.map((o, i) =>
    `        <label class="ws-pick"><input type="checkbox" disabled><span class="ws-pick-key">${"ABCD"[i] || "?"}</span>${escapeHtml(o)}</label>`).join("\n") : "";
  const trapText = q1 ? escapeHtml(q1.hint) : "先寫下一個你覺得「大部份的人都會搞錯」的地方，做完實驗回來看看自己有沒有猜對。";
  const num = n => `<h2><span class="ws-num">${n}</span>${""}</h2>`;
  ...
  const body = `  <div class="worksheet">
    ...
    <section class="worksheet-block ws-mission">
      <h2><span class="ws-num">1</span>本次任務</h2>
      <p class="ws-hook">打開模擬，操作、量測、作圖，然後用數據說服別人。以下三件事全部做體會到，任務就算過關：</p>
      <ul class="ws-goals">
${points}
      </ul>
    </section>
    ...
    <section class="worksheet-block">
      <h2><span class="ws-num">2</span>⚠ 大部份的人第一次都會栽在這裡</h2>
      <div class="ws-trap">
        <p>${trapText}</p>
      </div>
      <p class="ws-trap-note">先記住這個陷阱，做完實驗回來看看自己有沒有中招。</p>
    </section>

    <section class="worksheet-block">
      <h2><span class="ws-num">3</span>動手前，先預測（圈選你的答案）</h2>
${q1 ? `      <p>${escapeHtml(q1.prompt)}</p>
      <div class="ws-predict">
${q1Opts}
      </div>
      <p class="worksheet-lines">我的理由：_______________________________________________________</p>` :
`      <p>先不要動手。看著實驗目標，寫下你預期會看到什麼現象、哪個量會變大：</p>
      <p class="worksheet-lines">我的預測：_______________________________________________________</p>`}
    </section>

    <section class="worksheet-block">
      <h2><span class="ws-num">4</span>操作步驟檢核（做到才打勾）</h2>
      <div class="ws-step"><span class="ws-box"></span>打開模擬，先只改<strong>一個</strong>變因，其他滑桿固定。</div>
      <div class="ws-step"><span class="ws-box"></span>記錄改變前的讀值，再慢慢改變它，觀察哪個量跟著變。</div>
      <div class="ws-step"><span class="ws-box"></span>換另一個變因重做一次，比較兩次的差別。</div>
      <div class="ws-step"><span class="ws-box"></span>把數據填進下面的表格，完成作圖與斜率計算。</div>
    </section>

    <section class="worksheet-block">
      <h2><span class="ws-num">5</span>資料記錄</h2>
      <p>每次只改變一個變因，其餘固定。把改變的量與量測到的量記在表格裡（單位記得先寫）。</p>
...
${rows}
    ...
    <section class="worksheet-block">
      <h2><span class="ws-num">8</span>一句話結論</h2>
      <p class="ws-hook">如果只能說一句話，你會怎麼跟沒做過這個實驗的人解釋你發現了什麼？</p>
...
/* ---------------------------------------------------------------------------
   ...
   --------------------------------------------------------------------------- */
function main() {
  const { curriculum, site, qbank } = loadCurriculum();
  ...
  flat.forEach((item, i) => {
    ...
    fs.writeFileSync(path.join(OUT_DIR, sheetName),
      worksheetPage(item.exp, item.mod, conf, qbank), "utf8");
