...
(function () {
  ...
  const bank = {
    ...
    "rail-rods": [
      {
        type: "單選題 · 相對速度",
        prompt: "兩根導體棒在同一組平行導軌上同向滑動：棒1 速度 3 m/s、棒2 速度 1 m/s，磁場垂直紙面。感應電動勢的大小取決於什麼？",
        formula: "\\( \\varepsilon=BL(v_1-v_2) \\)",
        options: [
          "兩棒的相對速度 2 m/s：ε = BL×2",
          "只看較快的棒1：ε = BL×3",
          "兩棒速度相加：ε = BL×4",
          "只看較慢的棒2：ε = BL×1"
        ],
        correct: 0,
        hint: "迴路面積的變化率由「兩棒拉開（或靠近）得多快」決定——把兩根棒的速度相減看看。",
        answer: "迴路在單位時間內增加的面積是 (v₁−v₂)L，所以 ε = BL(v₁−v₂)，只有「相對速度」算數。把兩棒速度相加是把方向搞反了：棒2 同向滑動其實在「追上」迴路面積的變化。在模擬裡讓兩棒速度接近，電流明顯變小——這就是相對速度在說話。"
      },
      {
        type: "單選題 · 收斂之後",
        prompt: "兩棒達到共同速度之後，迴路中的感應電流與安培力會如何？",
        options: [
          "電流歸零、安培力消失，兩棒一起等速前進",
          "電流維持最大值，安培力繼續加速慢棒",
          "電流反向，安培力使兩棒逐漸停下",
        ...
        correct: 0,
        hint: "產生感應電動勢的是磁通量的「變化」。兩棒同速時，迴路圍住的面積還在變嗎？",
        answer: "兩棒同速時迴路面積不再變化，磁通量恆定，電流與安培力同時歸零——兩棒以共同速度等速前進。這正是「感應電動勢只看變化率」的又一次出場；模擬裡 v–t 圖收斂成同一條水平線，I 讀數歸零。"
      },
      {
        type: "單選題 · 能量與動量",
        prompt: "從棒1獲得初速到兩棒達共同速度，系統減少的動能去哪裡了？過程中系統總動量為何守恆？",
        options: [
          "動能變成迴路的焦耳熱；兩棒所受安培力等大反向，系統合外力為零",
          "動能變回重力位能；因為磁場對系統做正功",
          "動能憑空消失；動量守恆是因為質量也減少了",
          "動能全部轉給磁場儲存，之後自動還給兩棒"
        ],
        correct: 0,
        hint: "兩棒受到的安培力都來自同一個磁場，大小都是 BIL——它們的方向呢？",
        answer: "安培力使快棒減速、慢棒加速，兩力等大反向（同一電流、同一磁場、同長導體），系統合外力為零，所以動量守恆、最後共同速度 v* = m₁v₁₀/(m₁+m₂)。減少的動能 ∫I²R dt 正是電流流過電阻產生的焦耳熱——「電磁煞車」煞掉的每一分動能都變成了熱，模擬的 Q 讀數可以對帳。"
      }
    ]
  };
  ...
  const numericBank = {
    ...
    "rail-rods": {
      ...
      unit: "m/s",
      tolerance: 0.03,
      hint: "導軌光滑、安培力是系統內力，動量守恆：m₁v₀ = (m₁+m₂)v*。",
      generate(rand) {
        const m1 = rand(0.4, 2.0, 0.2);
        const m2 = rand(0.6, 2.4, 0.2);
        const v0 = rand(2, 6, 0.5);
        const vStar = m1 * v0 / (m1 + m2);
        const Q = 0.5 * m1 * v0 * v0 - 0.5 * (m1 + m2) * vStar * vStar;
        return {
          prompt: "質量 " + m1.toFixed(1) + " kg 的導體棒在光滑平行導軌上以 " + v0.toFixed(1) +
            " m/s 滑向前方 " + m2.toFixed(1) + " kg、原本靜止的另一根棒（磁場垂直紙面，兩棒經導軌構成迴路）。" +
            "求兩棒達共同速度後的速度 v*，以及過程中迴路產生的焦耳熱 Q。",
          answer: vStar,
          steps: [
            "安培力是兩棒之間的一對交互作用（等大反向），系統合外力為零 → 動量守恆",
            "m₁v₀ = (m₁+m₂)v* → v* = " + m1.toFixed(1) + " × " + v0.toFixed(1) + " ÷ " + (m1 + m2).toFixed(1) + " = " + vStar.toFixed(3) + " m/s",
            "焦耳熱 = 動能損失 = ½m₁v₀² − ½(m₁+m₂)v*² = " + Q.toFixed(3) + " J",
            "（檢查題目問的是速度還是熱——兩個答案都在上面）"
          ]
        };
      }
    },
