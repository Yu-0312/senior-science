...
(function () {
  ...
  const PROCEDURE = {
    ...
    "heat-engine-principle": {
      ...
      steps: [
        "先用「課本標準」預設播一次：注意壓力條要<strong>累積到阻力門檻</strong>才會噴，不是一加熱就噴。",
        "換「弱火慢熱」與「強火快噴」：比較壓力爬升速度——火力決定內能累積快慢，塞子緊度決定門檻高低。",
        "塞子射出瞬間盯住管口：<strong>白霧出現</strong>與<strong>內能下降／機械能上升</strong>是同一件事的兩面。",
        "讀因果面板：火焰到內能是「熱傳遞」，內能到塞子是「做功」——兩種改變內能的方式不要混為一談。",
        "想一想：為什麼這個演示<strong>不能</strong>叫完整熱機？（沒有循環、沒有把塞子送回起點、廢熱沒有被利用）"
      ],
      rule: "迷思鐵律：<strong>白霧不是水蒸氣</strong>（蒸氣不可見，白霧是液化小液滴）；" +
        "內能也<strong>不會 100% 變成機械能</strong>（器壁吸熱與散熱必然存在）。" +
        "108 對應：國中自然「物質與能量／能源」→ 高中必修能量守恆 → 選修熱力學與本站 heat-engine。"
    },
    "circuit-sandbox": {
  ...
  /* =====================================================================
     ...
     ===================================================================== */
  const CAUSALITY = {
    "measurement-error": { title: "隨機誤差 vs 系統誤差", rows: [
